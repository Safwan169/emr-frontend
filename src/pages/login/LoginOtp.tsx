...
const LoginOtp = () => {
  ...
  const [verifyLoginOtp, { isLoading, isError, error, isSuccess }] =
  ...
  const handleResend = () => {
    setOtp(["", "", "", "", "", ""]);
    setTimer(75);
    // Optional: trigger OTP resend API here
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      console.log("Verifying login OTP:", otp_code);
      const response = await verifyLoginOtp({ otp_code }).unwrap();
      console.log("Login OTP verified:", response);
