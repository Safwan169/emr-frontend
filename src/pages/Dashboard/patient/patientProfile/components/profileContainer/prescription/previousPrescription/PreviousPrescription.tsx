...
import { TPrevious } from "../../../../../../../../types/patientTypes";
...
const PreviousPrescription: React.FC = () => {
  ...
  // RTK Query hooks - Replace with your actual API endpoints
  const {
    data: prescriptionData,
    isLoading,
    isError,
    refetch,
  } = useGetPreviousPrescriptionByIdQuery(1); // Replace 1 with actual user ID or remove if getting all
  ...
  const transformedData: UploadedImage[] =
  ...
  // Handle file upload and convert to FormData for API
  const buildFormData = (file: File, description: string): FormData => {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("description", description);
    // Add user ID if required by your API
    formData.append("user_id", userId.toString());

    // Debug: Log FormData contents (TypeScript compatible way)
  ...
  const validateForm = (
    ...
    if (!data.file) {
      ...
      if (!allowedTypes.includes(data.file.type)) {
        errors.file = "Please select a valid image file (JPEG, PNG, GIF, WebP)";
      }
  ...
  // Start editing
  const startEdit = (image: UploadedImage) => {
